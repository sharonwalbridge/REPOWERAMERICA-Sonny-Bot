...
module.exports = (robot) ->
  ...
  robot.respond /quote (.+)/i, (msg) ->
    ...
    robot.http(googleUrl).query(googleQuery).get() (err, res, body) ->
      ...
      robot.http(surgeUrl).header('Content-Type', 'application/json').header('Authorization', 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..qXplDltC6ie6ovdfHS_on5Vqe2o5EySeDrg6HhegrL4').post(payload) (err, res, body) ->
        jsonBody = JSON.parse(body)
        quoteId = jsonBody.id
        if !quoteId
